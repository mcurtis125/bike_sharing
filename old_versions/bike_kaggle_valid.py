...
def main():
    ...
    X = min_max_scaler.fit_transform(np.array(filter_X(training_data), dtype = 'float_'))
    y = filter_y(training_data)
    #np.set_printoptions(threshold=np.nan)   
     
    # Split the dataset in two equal parts
    X_train_raw, X_test_raw, y_train_raw, y_test_raw = train_test_split(X, y, test_size=0.1, random_state=0)
    ...
    X_test = min_max_scaler.fit_transform(np.array(filter_X(X_test_raw), dtype = 'float_'))
    print X_test
    ...
    y_test_output = filter_predicted_data(y_test, y_test_raw)
    print y_test_output
    
    #write results to csv
    print "Writing predictions to bike_sharing_svm_linear.csv"
    results = open('bike_sharing_svm_linear_march_7.2.csv', 'w')
    ...
    for index in range (0,6493):
        ...
        results.write('%s,' % y_test_raw[index][0])
